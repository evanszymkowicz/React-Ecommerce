...
class CategoryController {
  ...
  async store({view, auth, request, response, session}) {
    const validation = await validate(request.all(), {title: 'required|unique:categories,title'});
    if (validation.fails()) {
  ...
  async edit({view, auth, request, response, params}) {
    try {
      ...
      return view.render('admin/products/categories/edit_category.edge', {category});
    } catch (error) {
      console.log(error);
      return response.redirect('back');
    }
